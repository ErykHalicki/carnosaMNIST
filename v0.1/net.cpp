...
void network::sort(){
    for(int i = 1;i < connectionNum; i++) {
        int j = i;
        while(j > 0 && (connections[j].src&15) < (connections[j-1].src&15)) {
...
void network::init(int l, int ninl,int randomConnections){
    ...
    for(int i=0;i<randomConnections;i++){
        ...
        connections[i].src=createID(src_layer,src_id);
        connections[i].dest=createID(dest_layer,dest_id);
    }
    sort();
}
void network::clear(){
...
void network::run(unsigned char* input, float* result){
    ...
    short src_id,dest_id;
    unsigned char src_layer, dest_layer;
    float srcValue,weight;
    for(int i=0;i<connectionNum;i++){
        ...
        neurons[dest_layer][dest_id].value=std::fmax(0,neurons[dest_layer][dest_id].value+srcValue*weight);
    }
    float sum=0;
...
void reproduce(network n1, network n2,network* offspring){
    int halfConnections=rand()%n1.connectionNum/2;
    ...
    memcpy(&offspring[0].connections[halfConnections],&n2.connections[halfConnections],sizeof(connection)*(n1.connectionNum-halfConnections)); 
