...
float evaluate(network net,uchar* labels,int setSize, uchar** images){
    float accuracy=0;
...
void evolve(int generations, int population,network* result){
    ...
    for(int gen=0;gen<generations;gen++){
        ...
        for(int n=0;n<population;n++){
            accuracies[n]=evaluate(networks[n],labels,100,images);
        }  
        ...
        for(int i=2;i<population-4;i++){
            networks[i].randomize();//every net not being reproduced gets slightly randomized
        }
        //doesnt seg fault or memory leak after commenting out reproduction
    }
    memcpy(result,networks,sizeof(network));
}
int main(int argc, char** argv){
    srand (time(NULL));
    std::cout<<"Carnosa_MNIST v0.1\n";
